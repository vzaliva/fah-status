...
type lexer_state = Yojson.Lexer_state.t = {
  buf : Bi_outbuf.t;
  mutable lnum : int;
  mutable bol : int;
...
val stream_from_string :
  ...
  ?fname:string ->
  ?lnum:int ->
  string -> Yojson.Basic.t Stream.t
...
val stream_from_channel :
  ...
  ?fname:string ->
  ?lnum:int ->
  in_channel -> Yojson.Basic.t Stream.t
...
val stream_from_lexbuf :
  lexer_state ->
  ?fin:(unit -> unit) ->
  Lexing.lexbuf -> Yojson.Basic.t Stream.t
...
type json_line = [ `Json of Yojson.Basic.t | `Exn of exn ]
...
val read_t : lexer_state -> Lexing.lexbuf -> Yojson.Basic.t
...
val finish_variant : lexer_state -> Lexing.lexbuf -> Yojson.Basic.t option
...
val read_json : lexer_state -> Lexing.lexbuf -> Yojson.Basic.t
val skip_json : lexer_state -> Lexing.lexbuf -> unit
val buffer_json : lexer_state -> Lexing.lexbuf -> unit

val validate_json : 'path -> Yojson.Basic.t -> 'error option
  (* always returns [None].
     Provided so that atdgen users can write:

       type Yojson.Basic.t <ocaml module="Yojson.Safe"> = abstract
  *)
